...
describe('mask draft lifecycle in store actions', () => {
  ...
  it('does not apply the outer watchdog to concurrent Codex CLI custom requests', async () => {
    ...
    expect(useStore.getState().tasks[0]).toMatchObject({
      outputErrors: [{ requestIndex: 1, error: '图片生成失败：The operation was aborted' }],
    })
...
describe('fal task recovery', () => {
  ...
  it('records recovered Agent tool failures without continuing the Agent round', async () => {
    ...
    await initStore()
    await vi.waitFor(() => {
      expect(useStore.getState().agentConversations[0]?.rounds[0]).toMatchObject({ status: 'error', error: '账户额度不足，请充值或更换 API 密钥' })
    })
