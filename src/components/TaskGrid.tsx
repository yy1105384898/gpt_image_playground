...
export default function TaskGrid() {
  ...
  if (!filteredTasks.length) {
    return (
      <div className="yy-empty-state text-center text-gray-400">
        {searchQuery || filterFavorite ? (
          <p className="text-sm">没有找到匹配的任务</p>
        ) : (
          <>
            <svg
              className="w-20 h-20 mx-auto mb-5 text-white/18"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <p className="text-base font-medium text-gray-300">输入提示词开始生成图片</p>
          </>
        )}
      </div>
    )
