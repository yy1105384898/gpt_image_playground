...
export default function InputBar() {
  ...
  return (
    <>
      ...
      <div
        data-input-bar
        className={`fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-30 w-full max-w-5xl px-3 sm:px-4 transition-all duration-300${promptExpanded ? ' flex flex-col' : ''}`}
      ...
      >
        ...
        />
        <div ref={cardRef} className={`bg-[#101012]/92 backdrop-blur-2xl border border-white/[0.09] shadow-[0_24px_80px_rgba(0,0,0,0.5)] rounded-[24px] p-3 sm:p-4 ring-1 ring-white/[0.05]${promptExpanded ? ' flex min-h-0 flex-1 flex-col' : ''}`}>
          {/* 移动端拖动条 */}
          <div
            ref={handleRef}
